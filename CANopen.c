...
#if defined(CO_USE_GLOBALS)
#if !defined(CO_TRACE_BUFFER_SIZE_FIXED)
#define CO_TRACE_BUFFER_SIZE_FIXED 100
#endif /* !defined(CO_TRACE_BUFFER_SIZE_FIXED) */
#endif /* defined(CO_USE_GLOBALS) */
#endif /* CO_NO_TRACE > 0 */

/* Verify features from CO_OD *************************************************/
/* generate error, if features are not correctly configured for this project */
#if CO_NO_NMT_MASTER > 1 || CO_NO_SYNC != 1 || CO_NO_EMERGENCY != 1 || CO_NO_SDO_SERVER == 0 \
    || CO_NO_SDO_CLIENT > 128 || (CO_NO_RPDO < 1 || CO_NO_RPDO > 0x200)                      \
    || (CO_NO_TPDO < 1 || CO_NO_TPDO > 0x200) || ODL_consumerHeartbeatTime_arrayLength == 0  \
    || ODL_errorStatusBits_stringLength < 10 || CO_NO_LSS_SERVER > 1 || CO_NO_LSS_CLIENT > 1 \
    || (CO_NO_LSS_SERVER > 0 && CO_NO_LSS_CLIENT > 0)
#error Features from CO_OD.h file are not correctly configured for this project!
#endif /* Compile-time checks */

/* Indexes for CANopenNode message objects ************************************/
#if defined(ODL_consumerHeartbeatTime_arrayLength)
#define CO_NO_HB_CONS ODL_consumerHeartbeatTime_arrayLength
#else
#define CO_NO_HB_CONS 0
#endif                  /* defined(ODL_consumerHeartbeatTime_arrayLength) */
...
#define CO_RXCAN_SDO_SRV \
    (CO_RXCAN_RPDO + CO_NO_RPDO) /*  start index for SDO server message (request) */
#define CO_RXCAN_SDO_CLI \
    (CO_RXCAN_SDO_SRV + CO_NO_SDO_SERVER) /*  start index for SDO client message (response) */
#define CO_RXCAN_CONS_HB \
    (CO_RXCAN_SDO_CLI + CO_NO_SDO_CLIENT) /*  start index for Heartbeat Consumer messages */
...
#define CO_RXCAN_NO_MSGS                                                                 \
    (1 + CO_NO_SYNC + CO_NO_EMERGENCY + CO_NO_RPDO + CO_NO_SDO_SERVER + CO_NO_SDO_CLIENT \
     + CO_NO_HB_CONS)
...
#define CO_TXCAN_SDO_SRV \
    (CO_TXCAN_TPDO + CO_NO_TPDO) /*  start index for SDO server message (response) */
#define CO_TXCAN_SDO_CLI \
    (CO_TXCAN_SDO_SRV + CO_NO_SDO_SERVER) /*  start index for SDO client message (request) */
...
#define CO_TXCAN_NO_MSGS                                                             \
    (CO_NO_NMT_MASTER + CO_NO_SYNC + CO_NO_EMERGENCY + CO_NO_TPDO + CO_NO_SDO_SERVER \
     + CO_NO_SDO_CLIENT + CO_NO_HB_PROD + CO_NO_LSS_SERVER + CO_NO_LSS_CLIENT)
...
CO_ReturnError_t CO_new(void)
{
    ...
    if (CO == NULL) { /* Use malloc only once */
        ...
        CO_SDO_ODExtensions =
            (CO_OD_extension_t*) calloc(CO_OD_NoOfElements, sizeof(CO_OD_extension_t));
    ...
    CO_memoryUsed = sizeof(CO_CANmodule_t) + sizeof(CO_CANrx_t) * CO_RXCAN_NO_MSGS
                    + sizeof(CO_CANtx_t) * CO_TXCAN_NO_MSGS + sizeof(CO_SDO_t) * CO_NO_SDO_SERVER
                    + sizeof(CO_OD_extension_t) * CO_OD_NoOfElements + sizeof(CO_EM_t)
                    + sizeof(CO_EMpr_t) + sizeof(CO_NMT_t) + sizeof(CO_SYNC_t)
                    + sizeof(CO_RPDO_t) * CO_NO_RPDO + sizeof(CO_TPDO_t) * CO_NO_TPDO
                    + sizeof(CO_HBconsumer_t) + sizeof(CO_HBconsNode_t) * CO_NO_HB_CONS
#if CO_NO_LSS_SERVER == 1
    ...
    errCnt = 0;
    if (CO->CANmodule[0] == NULL)
        errCnt++;
    if (CO_CANmodule_rxArray0 == NULL)
        errCnt++;
    if (CO_CANmodule_txArray0 == NULL)
        errCnt++;
    for (i = 0; i < CO_NO_SDO_SERVER; i++) {
        if (CO->SDO[i] == NULL)
            errCnt++;
    }
    if (CO_SDO_ODExtensions == NULL)
        errCnt++;
    if (CO->em == NULL)
        errCnt++;
    if (CO->emPr == NULL)
    ...
    if (CO->HBcons == NULL)
        errCnt++;
    if (CO_HBcons_monitoredNodes == NULL)
        errCnt++;
#if CO_NO_LSS_SERVER == 1
    if (CO->LSSslave == NULL)
        errCnt++;
...
#if CO_NO_NMT_MASTER == 1
    NMTM_txBuff =
        CO_CANtxBufferInit(/* return pointer to 8-byte CAN data buffer, which should be populated */
...
CO_NMT_reset_cmd_t CO_process(CO_t* CO, uint16_t timeDifference_ms, uint16_t* timerNext_ms)
{
    ...
    CO_EM_process(
        CO->emPr, NMTisPreOrOperational, timeDifference_ms * 10, OD_inhibitTimeEMCY, timerNext_ms);
    ...
    CO_HBconsumer_process(CO->HBcons, NMTisPreOrOperational, timeDifference_ms);
